...
public class UserHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        if(userRepository.findAll().size() > 3) {
            return new Health.Builder().up().withDetail("users", "we have more than 3 users in our database").build();
        } else {
            return new Health.Builder().down().withDetail("users", "we have less than 3 users in our database").build();
        }
    }
}
